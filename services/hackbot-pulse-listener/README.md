...
5. Trigger the agent with the failing tasks -- it resolves the push commits itself. The
   listener also reads the push's `json-pushes` pushlog to keep a commit -> author map
   locally, so it can email the developer whose commit the agent blames.
6. `POST /agents/build-repair/runs`, then poll `GET /runs/{run_id}` until terminal and send
   the report email.
...
is logged and skipped. Notifications go to the blamed commit's author, then the pushing
developer, and, if set, the `NOTIFICATION_TEAM_EMAIL` team address. Set `NOTIFICATION_OVERRIDE_EMAIL` to route every notification to a
